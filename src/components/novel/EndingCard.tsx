...
type Props = { state: Pick<GameState, "flags" | "affinity">; onReturnToTitle: () => void; onRestart: () => void }

export function EndingCard({ state, onReturnToTitle, onRestart }: Props) {
  ...
  return <section className="end-card end-card--complete" aria-label="エンディング">
    ...
    <div className="end-card__actions"><button type="button" onClick={onReturnToTitle}>タイトルへ</button><button type="button" onClick={onRestart}>もう一度読む</button></div>
  </section>
}
